...
class TestSubprocessGradle(TestCase):
    ...
    def test_no_os_utils_build_init_throws(self):
        with self.assertRaises(ValueError) as err_assert:
            SubprocessGradle(self.gradle_binary)
    ...
    def test_build_no_init_script(self):
        ...
        self.os_utils.popen.assert_called_with([self.gradle_path, 'build'], cwd=self.source_dir, stderr=subprocess.PIPE,
    ...
    def test_includes_build_properties_in_command(self):
        gradle = SubprocessGradle(self.gradle_binary, self.os_utils)
        gradle.build(self.source_dir, self.init_script, {'foo': 'bar'})
